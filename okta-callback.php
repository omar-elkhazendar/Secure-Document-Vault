...
require_once 'classes/OktaAuth.php';
...
if (isset($_GET['code'])) {
    ...
    $user = $oktaAuth->handleCallback($_GET['code']);
    ...
    if ($user) {
        ...
        if ($session->createSession($user->id)) {
            ...
            $session->set('auth_method', 'okta');  // Set the auth method
        ...
        error_log("Okta authentication failed - No user data returned");
        header("Location: login.php?error=okta_auth_failed");
        exit();
    }
} else if (isset($_GET['error'])) {
    error_log("Okta OAuth Error: " . $_GET['error']);
    if (isset($_GET['error_description'])) {
        error_log("Error Description: " . $_GET['error_description']);
    }
    header("Location: login.php?error=okta_auth_error");
    exit();
}

// If we get here, something went wrong
error_log("Okta callback reached without code or error parameter");
header("Location: login.php?error=okta_auth_failed");
exit(); 